...
def setup():
    con = sqlite3.connect('jovandb.db')
    cur = con.cursor()
    ...
    con.commit()
    cur.execute(
        "CREATE TABLE if NOT exists alertas (timestamp DATETIME,sid INTEGER, msg TEXT,clasificacion TEXT,prioridad INTEGER,protocolo TEXT,origen TEXT,destino TEXT,puerto TEXT, FOREIGN KEY(destino) REFERENCES devices(ip),FOREIGN KEY(origen) REFERENCES devices(ip))")
    cur.execute(
        "CREATE TABLE IF NOT EXISTS devices (id TEXT, ip TEXT, localizacion TEXT, PRIMARY KEY('id'))")
    cur.execute(
        "CREATE TABLE IF NOT EXISTS responsable (id TEXT, nombre TEXT, telefono TEXT, ROL TEXT, PRIMARY KEY('id'), FOREIGN KEY('id') REFERENCES devices('id'))")
    cur.execute(
        "CREATE TABLE IF NOT EXISTS analisis (id TEXT, servicios INTEGER, servicios_inseguros INTEGER, vulnerabilidades_detectadas INTEGER ,PRIMARY KEY('id'),FOREIGN KEY('id') REFERENCES devices('id'))")
    cur.execute(
        "CREATE TABLE IF NOT EXISTS puertos_abiertos (id TEXT, puerto TEXT, FOREIGN KEY('id') REFERENCES analisis('id'))")
    con.commit()
...
def practica1():
    ...
    #print(df_puertos_abiertos)
    df_aux = df_puertos_abiertos["ID"].value_counts()
    print("Media y desviacion estandar del total de puertos abiertos: " + str(df_aux.mean()) + " || " + str(df_aux.std()))
    df_analisis = pd.DataFrame(cur.execute("SELECT * FROM analisis").fetchall(), columns=["ID", "SERVICIOS", "SERVICIOS_INSEGUROS", "VULNERABILIDADES_DETECTADAS"])
    df_aux = df_analisis["SERVICIOS_INSEGUROS"]
    print("Media y desviacion estandar del numero de servicios inseguros detectados: " + str(df_aux.mean()) + " || " + str(df_aux.std()))
    df_aux = df_analisis["VULNERABILIDADES_DETECTADAS"]
    print("Media y desviacion estandar del numero de vulnerabilidades detectadas: " + str(df_aux.mean()) + " || " + str(df_aux.std()))
    df_aux = df_puertos_abiertos["ID"].value_counts()
    aux = None
    if None in df_puertos_abiertos["PUERTO"].values: # esta parte es necesaria porque cuenta None como un valor normal cuando no lo es
        aux = 0
    else:
        aux = df_aux.min()
    print("Valor minimo y valor maximo del total de puertos abiertos: " + str(aux) + " || " + str(df_aux.max()))
    df_aux = df_analisis["VULNERABILIDADES_DETECTADAS"]
    print("Valor minimo y valor maximo del numero de vulnerabilidades detectadas: " + str(df_aux.min()) + " || " + str(df_aux.max()))

    # EJERCICIO 3

    print("\nEJERCICIO 3")

    df_alertas_graves_julio = pd.DataFrame(cur.execute("SELECT * FROM alertas WHERE prioridad = 1 AND strftime('%m', timestamp) = '07'").fetchall(), columns=["TIME", "SID", "MSG", "CLASIFICACION", "PRIORIDAD", "PROTOCOLO", "ORIGEN", "DESTINO", "PUERTO"])
    df_alertas_graves_agosto = pd.DataFrame(cur.execute("SELECT * FROM alertas WHERE prioridad = 1 AND strftime('%m', timestamp) = '08'").fetchall(), columns=["TIME", "SID", "MSG", "CLASIFICACION", "PRIORIDAD", "PROTOCOLO", "ORIGEN", "DESTINO", "PUERTO"])
    df_alertas_medias_julio = pd.DataFrame(cur.execute("SELECT * FROM alertas WHERE prioridad = 2 AND strftime('%m', timestamp) = '07'").fetchall(), columns=["TIME", "SID", "MSG", "CLASIFICACION", "PRIORIDAD", "PROTOCOLO", "ORIGEN", "DESTINO", "PUERTO"])
    df_alertas_medias_agosto = pd.DataFrame(cur.execute("SELECT * FROM alertas WHERE prioridad = 2 AND strftime('%m', timestamp) = '08'").fetchall(), columns=["TIME", "SID", "MSG", "CLASIFICACION", "PRIORIDAD", "PROTOCOLO", "ORIGEN", "DESTINO", "PUERTO"])
    df_alertas_bajas_julio = pd.DataFrame(cur.execute("SELECT * FROM alertas WHERE prioridad = 3 AND strftime('%m', timestamp) = '07'").fetchall(), columns=["TIME", "SID", "MSG", "CLASIFICACION", "PRIORIDAD", "PROTOCOLO", "ORIGEN", "DESTINO", "PUERTO"])
    df_alertas_bajas_agosto = pd.DataFrame(cur.execute("SELECT * FROM alertas WHERE prioridad = 3 AND strftime('%m', timestamp) = '08'").fetchall(), columns=["TIME", "SID", "MSG", "CLASIFICACION", "PRIORIDAD", "PROTOCOLO", "ORIGEN", "DESTINO", "PUERTO"])
    df_dispositivos = pd.DataFrame(cur.execute("SELECT * FROM devices").fetchall(), columns=["ID", "IP", "LOCALIZACION"])

    df_aux = pd.merge(df_alertas_graves_julio, df_dispositivos, left_on="ORIGEN", right_on="IP", how="inner")
    df_aux2 = pd.merge(df_alertas_graves_julio, df_dispositivos, left_on="DESTINO", right_on="IP", how="inner")
    ...
    print("Numero de valores ausentes: " + str(df_aux.isna().sum().sum()))
    print("Mediana: " + str(df_aux.median()))
    print("Media: " + str(df_aux.mean()))
    print("Varianza: " + str(df_aux.var()))
    print("Valores maximo y minimo: " + str(df_aux.min()) + " || " + str(df_aux.max()))
    ...
    df_alertas = pd.DataFrame(cur.execute("SELECT * FROM alertas").fetchall(), columns=["TIME", "SID", "MSG", "CLASIFICACION", "PRIORIDAD", "PROTOCOLO", "ORIGEN", "DESTINO", "PUERTO"])
