...
def setup():
    con = sqlite3.connect('practicaDB.db')
    cur = con.cursor()
...
def practica1():
    ...
    print("\nValores ausentes:\n" + str(df_aux.LOCALIZACION.isna().groupby([df_aux["PRIORIDAD"], df_aux["MES"]]).sum()))
    ...
    grafico = px.bar(df_alertas_41["ORIGEN"].value_counts().head(10), y="ORIGEN", title="10 IP de origen mas problematicas")
    #grafico.show()
    ...
    grafico = px.line(df_alertas_42, x="TIME", title="Numero de alertas en el tiempo")
    #grafico.show()
    ...
    grafico = px.bar(df_alertas_43["CLASIFICACION"].value_counts(), log_y=True, title="numero de alertas por categoria")
    #grafico.show()
    ...
    grafico = px.bar(df_analisis_44, x='ID', y="VULNERABLE", title="dispositivos mas vulnerables")
    #grafico.show()
